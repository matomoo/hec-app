...
const UpdateTaskId = ({
  ...
  if (taskId === undefined && (noSep !== undefined || noSep !== null)) {
    ...
    const waktuReg =
      dayjs().subtract(1, "day").format("DD-MM-YYYY") + " " + jamReg;
    const waktuForTid23 =
      dayjs(waktuReg, "DD-MM-YYYY HH:mm:ss", true).unix() * 1000;
    const res23 = sendTaskid(noBooking, 1, waktuForTid23);
    res23.then((a) => {
      if (a.includes("Tanggal pelayanan untuk Kode Booking tersebut adalah")) {
        console.log(
          "taskid undefined > send taskId 1 and noBooking " + noBooking,
        );
        const waktuReg =
          dayjs().subtract(0, "day").format("DD-MM-YYYY") + " " + jamReg;
        const waktuForTid23 =
          dayjs(waktuReg, "DD-MM-YYYY HH:mm:ss", true).unix() * 1000;
        const res23 = sendTaskid(noBooking, 1, waktuForTid23);
  ...
  if (taskId === 1 && minuteDiff > 1) {
    ...
    res23.then((a) => {
      if (
        a === "TaskId=2 tidak valid / TaskId sebelumnya belum terkirim" ||
        a === "TaskId=1 belum ada"
      ) {
        console.log("send taskId 1 in 2 and noBooking " + noBooking);

        const addMinut12 = Math.floor(Math.random() * 9) + 1;
        const addSecond12 = Math.floor(Math.random() * 60);

        const waktuForTid12 =
          dayjs(waktu, "DD-MM-YYYY HH:mm:ss", true)
            .subtract(addMinut12, "minute")
            .add(addSecond12, "second")
            .unix() * 1000;
  ...
  if (taskId === 2 && minuteDiff > 1) {
    ...
    const addSecond34 = Math.floor(Math.random() * 60);
    const waktuForTid4 =
      dayjs(waktu, "DD-MM-YYYY HH:mm:ss", true)
        .add(addMinut34, "minute")
        .add(addSecond34, "second")
        .unix() * 1000;
  ...
  if (taskId === 3 && minuteDiff > 5) {
    ...
    res.then((a) => {
      if (
        a === "TaskId=4 tidak valid / TaskId sebelumnya belum terkirim" ||
        a === "TaskId=3 belum ada"
      ) {
        ...
        res.then((a) => {
          if (a === "Task=1 ada. TaskId=2 belum ada") {
            console.log(
              "last tid = 3 but stuck. send taskId 2 and noBooking " +
                noBooking,
            );
            const addMinut23 = Math.floor(Math.random() * 15);
            const addSecond23 = Math.floor(Math.random() * 60);
  ...
  if (taskId === 4 && minuteDiff > 5) {
    ...
    const addMinut45 = Math.floor(Math.random() * 5) + 10;
    const addSecond45 = Math.floor(Math.random() * 60);

    const waktuForTid4 =
      dayjs(waktu, "DD-MM-YYYY HH:mm:ss", true)
        .add(addMinut45, "minute")
        .add(addSecond45, "second")
        .unix() * 1000;
  ...
  if (taskId === null || taskId === undefined) taskId === 98;

  return (
    <Tag color={taskId === 99 ? "default" : "processing"}>
      Last Task Id : {taskId}
    </Tag>
  );
};
